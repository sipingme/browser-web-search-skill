...
install:
  command: npx --yes browser-web-search@^0.3.5
  riskLevel: high
...
```bash
# 通过 npx 动态执行（无需全局安装）
npx --yes browser-web-search@^0.3.5 --version
...
```bash
npx --yes browser-web-search@^0.3.5 site list
```
...
- [ ] 安装工具：`npx --yes browser-web-search@^0.3.5 --version`
- [ ] 查看命令：`npx --yes browser-web-search@^0.3.5 site list`
- [ ] 测试运行：`npx --yes browser-web-search@^0.3.5 zhihu/hot`
- [ ] 看到 JSON 输出
